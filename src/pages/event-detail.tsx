...
import { BackIcon, CalendarIcon, LocationIcon } from "@/components/icons"
...
import { TYPE_ACCENTS, formatDateFull } from "@/lib/events"
...
function formatDateRange(dateStr: string, endDateStr?: string) {
  if (!endDateStr) return formatDateFull(dateStr)
  return `${formatDateFull(dateStr)} – ${formatDateFull(endDateStr)}`
}
      ...
      <div className="flex flex-wrap items-start gap-4">
        <div className="flex-1">
          <div className="flex items-center gap-2.5">
            <span
              className={`inline-flex items-center rounded-md px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wider ${accent.badge}`}
            >
