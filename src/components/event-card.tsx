  ...
  return (
    ...
    >
      ...
      <div className="p-4 pt-3">
        {/* Top row: type badge + secondary types + date */}
          ...
          <div className="flex min-w-0 items-center gap-2 overflow-hidden">
            ...
            {secondaryTypes.length > 0 && (
              <div className="flex items-center gap-1">
                {secondaryTypes.map((c) => (
                  <span
                    key={c}
                    title={EVENT_TYPE_LABELS[c]}
                    className={`inline-flex items-center gap-1 rounded-md px-1.5 py-0.5 text-[10px] font-medium ${TYPE_ACCENTS[c]?.badge ?? "bg-slate-100 text-slate-500"}`}
                  >
                    <span className={`size-1.5 rounded-full ${TYPE_ACCENTS[c]?.dot ?? "bg-slate-400"}`} />
                    {EVENT_TYPE_LABELS[c]}
                  </span>
                ))}
              </div>
        ...
        {/* Time + Location — improved contrast */}
        <div className="mt-1.5 flex flex-col gap-0.5">
          <span className="text-xs text-slate-600 dark:text-slate-400">{event.time}</span>
          <span className="flex min-w-0 items-center gap-1 text-xs text-slate-600 dark:text-slate-400">
            <LocationIcon />
        ...
        {/* Level + Tags — consolidated */}
        <div className="mt-3 flex flex-wrap items-center gap-1.5">
          <span className="rounded-md bg-slate-100 px-2 py-1 text-[11px] font-medium text-slate-700 dark:bg-slate-800 dark:text-slate-300">
            {event.level}
          </span>
          {event.tags.slice(0, 2).map((tag) => (
            <span
              key={tag}
              className="rounded-md bg-slate-50 px-2 py-1 text-[11px] text-slate-600 dark:bg-slate-800/50 dark:text-slate-400"
            >
              {tag}
            </span>
          ))}
          {event.tags.length > 2 && (
            <span className="rounded-md bg-slate-50 px-2 py-1 text-[11px] text-slate-500 dark:bg-slate-800/50 dark:text-slate-400">
      ...
      {/* Footer: organizer + attendees — improved contrast & tap targets */}
